...
# Load CodeBERT model, dataset and tokenizer
...
def mask_code(code, consider = ['variable_names','variable_values','strings','comments'],mask_ratio=1,count=0):
    if count>5:
        ...
        for i, token in enumerate(tokens):
            ...
            if random.random() < mask_ratio:
                ...
                if t:
                    ...
                    counter += 1
                # elif token in string_comment_words and len(token) > 3:
    ...
    if len(masked_values)==0:
        mask_code(code,consider=consider,mask_ratio=mask_ratio,count=count+1)
...
def t5_get_code(code_snippet,max_length=1024):
    ...
    input_ids = t5_tokenizer.encode(code_snippet, return_tensors='pt', truncation=True).to(device)
...
def get_code(masked_code):
    ...
    max_length = model.config.max_position_embeddings - 2  # Adjust for special tokens
    predicted_tokens = []
...
for i in range(4000):
    ...
    for instance in tqdm(dataset['train'].select(range(100*i,(i+1)*100))):
        try:
            ...
            instance['masked_code'] = masked_code
