...
bleu={'codebleu':np.array([]),'ngram_match_score':np.array([]),'weighted_ngram_match_score':np.array([]),'syntax_match_score':np.array([]),'dataflow_match_score':np.array([]),'variable_match_score':np.array([])}
...
for i in range(4000):
    ...
    print('Variable Match Score:', np.mean(bleu['variable_match_score']))
    print('-'*50)